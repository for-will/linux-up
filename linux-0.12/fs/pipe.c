...
// 如果需要读取的字节计数count大于0，我们循环执行以下操作。在循环读操作过程中，
// 若当前管道中没有数据（size=0），则唤醒等待该节点的进程，这通常是写管道进程。如果
// 已没有写管道者，即i节点引用计数值小于2，则返回已读字节数退出。如果当前收到有非
// 阻塞信号，则立刻返回已读取字节数退出；若还没有收到任何数据，则返回重新启动系统
// 调用号退出。否则就让进程在该管道上睡眠，用以下等待信息的到来。宏PIPI_SIZE定义在
...
// 此时说明管道（缓冲区）中有数据。于是我们取管道尾指针到缓冲区末端字节数chars。
...
// 如果当前管道中已经满了（空闲空间size = 0），则唤醒等待该管道的进程，通常唤醒
...
// PIPE_HEAD()定义在文件include/linux/fs.h中。
	while (count > 0) {
...
// written中。
		...
		count -= chars;
		written += chars;
// 再令size指向管道数据头指针处，并高速当前管道数据头部指针（前移chars字节）。若头
// 指针超过管道末端则绕回。然后从用户缓冲区复制chars个字节到管道头指针开始处。对于
// 管道i节点，其i_sizw字段中是管道缓冲块指针。
...
/// 创建管道系统调用。
// 在files所指的数组中创建一对文件句柄（描述符）。这对文件句柄指向一管道i节点。
// 参数：files - 文件柄数组。filedes[0]用于读管道数据，fildes[1]向管道写入数据。
// 成功时返回0，出错时返回-1。
...
// 首先从系统文件表中取两个空闲项（引用计数字段为0的项），并别设置引用计数为1。
...
// 冲区。如果不成功，则相应释放两个文件句柄和文件结构项，并返回-1。
	if (!(inode = get_pipe_inode())) {	// fs/inode.c，第231行开始处。
...
/// 管道io控制函数。
// 参数：pino - 管道i节点指针；cmd - 控制命令；arg - 用户参数。
// 函数返回0表示执行成功，否则返回出错码。
