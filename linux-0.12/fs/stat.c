...
// 首先验证（或分配）存放数据的内存空间。然后临时复制相应节点上的信息。最后交这些状态信息
// 复制到用户缓冲区中。
	verify_area(statbuf, sizeof(struct stat));
	tmp.st_dev = inode->i_dev;		// 文件所在的设备号。
	tmp.st_ino = inode->i_num;		// 文件的i节点号。
	tmp.st_mode = inode->i_mode;		// 文件属性。
	tmp.st_nlink = inode->i_nlinks;		// 文件链接数。
...
// 首先根据文件名找出对应的i节点，然后将i将i节点上的文件状态信息复制到用户缓冲区中，
// 并放回该i节点。
...
/// 符号链接文件状态系统调用。
// 根据给定的文件名获取相关文件状态信息，文件路径名中有符号链接文件名，则取符号链接文件
// 本身的状态，不跟随链接查目标文件。
// 参数filename是指定的文件名，statbuf是存放状态信息的缓冲区指针。
// 返回：成功返回0，若出错则返回出错码。
...
// 并放回该i节点。
	if (!(inode = lnamei(filename)))	// 取指定路径名i节点，不跟随管住嘴链接。
		return -ENOENT;
	cp_stat(inode, statbuf);
	iput(inode);
	return 0;
...
/// 使用文件句柄获取文件状态系统调用。
// 根据给定的文件句柄获取相关文件状态信息。
// 参数fd是指定文件的句柄（描述符），statbuf是存放状信息的缓冲区指针。
// 返回：成功返回0，若出错则返回出错码。
...
// 首先取文件句柄对应的文件结构，然后从中得到文件的i节点。然后将i节点上的文件状态信息
// 复制到用户缓冲区中。如果文件句柄值大于一个程序最多打开文件数NR_OPEN，或者该句柄的文
// 伯结构指针为空，或者对应文件结构的i节点字段为空，则出错，返回出错码并退出。
...
/// 读符号链接文件系统调用。
// 该调用读取符号链接文件的内容（即该符号链接所指向文件的路径名字符串），并放到指定长度
// 的用户缓冲区中。若缓冲区太小，就会截断符号链接的内容。
...
// 首先检查和验证函数参数的有效性，并对其进行调整。用户缓冲区长度bufsiz必须在1--1023
// 之间。然后取得符号链接文件名的i节点，并读取该文件的第1块数据内容。之后放回i节点。
	if (bufsiz <= 0)
		return -EBADF;
	if (bufsiz > 1023)
		bufsiz = 1023;
	...
	if (inode->i_zone[0])
		...
		bh = NULL;
	iput(inode);
// 如果成功读取文件数据内容，则从内容中复制最多bufsiz个字符到用户缓冲区中。不复制
