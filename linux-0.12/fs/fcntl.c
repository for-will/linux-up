...
extern int sys_close(int fd);	// 关闭文件系统调用。（fs/open.c，219）
...
{
// 首先检查函数的有效性。如果文件句柄值大于一个程序最多打开文件数NR_OPEN，或者
...
// 找到的新句柄值arg大于最多打开文件数（即没空闲项），则返回出错码并退出。
	while (arg < NR_OPEN)
		if (current->filp[arg])
			arg++;
		else
...
// 同的命令有不同的含义。对于复制句柄命令F_DUPFD，arg是冷热文件句柄可取的最小值；对
...
// 首先检查给出的文件句柄的有效性，然后根据不同命令cmd进行盆架处理。如果文件句柄
