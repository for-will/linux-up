 ...
 *
 * Virtual Consoles, Screnn Blanking, Screen Dumping, Colors, Graphics
 *   Chars, and VT100 enhancements by Peter MacDonald.
 */

/*
 *  NOTE!!! We sometimes disable and enable interrupts for short while
...
#include <linux/tty.h>		// tty头文件，定义有关tty_id，串行通信方面的参数、常数。
...
#define VIDEO_TYPE_EGAM 0x20	/* EGA/VAG in Monochrome Mode */ /* EGA/VAG单色 */ 
...
// 定义虚拟控制台结构，它包含一个虚拟控制台的当前所有信息。其中vc_origin和vc_scr_end
...
static struct {
	...
	unsigned long	vc_top,vc_bottom;	// 滚动时项行行号；底行行号。
...
#define def_attr	(vc_cons[currcons].vc_attr)
...
// 定义使用的字符集。其中上半部分是普通7比特ASCII代码，即US字符集。下半部分对应
...
//// 跟踪光标当前位置。
// 参数：currcons - 当前虚拟终端号；new_x - 梁村所在列号；new_y - 光标所在行号。
// 该函数用于更新当前光标位置变量 x,y，并修正光标在显示内存中的对应位置pos。该函数会
// 首先检查参数的有效性。如果给定的光标列号超出显示器最大列数，或者光标行号不低于显示的
...
//// 设置滚屏起始显示内存地址。
// 首先检查显示卡类型是否是彩色卡，并判断参数指定的控制台是否前台控制台，如果不满足这两
// 条件则退出。否则就向显示卡上寄存器输出滚屏起始地址。
static inline void set_origin(int currcons)
{
// 首先判断显示卡类型。对于 EGA/VGA 卡，我们可以指定屏内行范围（区域）进行滚屏操作，
// 而MDA单色显示卡只能进行整屏操作。因此只有EGA/VGA卡才需要设置滚屏起始行显示
// 内存地址（起始行是origin对应的行），即显示类型如果不是EGA/VGA彩色模式，也不是
// EGA/VGA单色模式，那么就直接返回。另外，我们只对前台控制台进行操作，因此只有当前控
// 制台currcons是前台控制台时，我们才需要设置其滚屏起始行对应的内存起点位置。
...
// 起始位置video_mem_base进行操作，例如对于EGA/VGA彩色模式，video_mem_base = 物理
// 内存地址0xb8000。
	cli();
...
//// 显示内容身上卷动一行。
// 将屏幕滚动窗口向下移一行，并在屏幕滚动区域底出现的新行上添加空格字符。滚屏区域
// 必须大于1行。关于滚屏操作原理，参见程序列表后的10.3.3.2节。
static void scrup(int currcons)
{
// 滚屏区域必须起码有2行。如果滚屏区域项行号大于等于区域底行号，则不满足进行滚行操作
// 的条件。另外，对于EGA/VGA卡，我们可以指定屏内行范围（区域）进行滚屏操作，而DMA单
// 色显卡只能进行整屏滚屏操作。该函数对EGA和MDA显示类型进行分别处理。如果显示类型
// 是EGA，则还分为整屏窗口移动和区域窗口移动。这里首先处理显示卡EGA/VGA显示类型
// 的情况。
	if (bottom <= top) 
		return;
	if (video_type == VIDEO_TYPE_EGAC || video_type == VIDEO_TYPE_EGAM) {
// 如果移动起始行 top=0，移动最底行 bottom = video_num_lines = 25，则表示整屏窗口向下
// 移动一行。于是把整个屏幕窗口左上角对应起始内存位置origin调整为向下移一行对应的
// 内存位置，同时也跟踪调整当前光标对应的内存位置以及屏幕末行末端字符指针scr_end的
// 位置。最后把新屏幕窗口内存起始位置值origin写入显示控制器中。
		if (!top && bottom == video_num_lines) {
			origin += video_size_row;
			pos += video_size_row;
			scr_end += video_size_row;
// 如果屏幕窗口末端所对应的显示内存指针scr_end超出了实际显示内存末端，则将屏幕内容
// 除第一行以外所有行对应的内存数据移动到显示内存起始位置video_mem_start处，并在
// 整屏窗口向下移动一行后出现的新行上填入空格字符。然后根据屏幕内存数据移动后的情况，
// 重新调整当前屏幕对应内存的起始指针、光标位置指针和屏幕末对应内存指针scr_end。
...
// 然后把新屏幕滚动窗口内存起始位置值origin写入显示控制器中。
			set_origin(currcons);
// 否则表示不是整屏移动。即表示从指定行top开始到bottom区域中的所有行身上移动1行，
// 指定行top被删除。此时直接将屏幕从指定行top到bottom所有行对应的显示内存数据向
// 上移动1行，并在最下面新出现的行上填入擦除字符。
// %0 - eax（擦除字符+属性）；%1 - ecx（从top+1行开始到bottom行所对应的内存长字数）；
...
// 并且会对超出显示内存范围的情况自动调整（即会自动翻卷指针），所以这里不对屏幕内容对应
// 内存超出显示内存范围的情况单独处理。处理方法与EGA非整屏移动情况完全一样。
	else {					/* Not EGA/VGA */
		__asm__("cld\n\t"
			"rep\n\t"
			"movsl\n\t"
...
//// 显示内容向下卷动一行。
// 将屏幕滚动窗口身上移动一行，相应屏幕滚动区域内容向下移动1行。移动开始行的上方会出现
// 一新行。参见程序列表后说明。处理方法与scrup()相似，只是为了在移动显示内存数据时不会
// 出现数据覆盖的问题，复制操作是以逆向进行的，即先从屏幕倒数第2行的最后一个字符开始复
// 制到最后一行，再将倒数第3行复制到倒数第2行等等。因此此时对EGA/VGA显示类型和MDA类型
// 型的处理过程完全一样，所以该函数实际上没有必要写成两段相同的代码，即这里if和else语句
// 块中的操作完全一样！
static void scrdown(int currcons)
{
// 同样地，滚屏区域必须起码有2行。如果滚屏区域项行号大于等于区域底行号，则不满足进行滚
...
//// 光标在同列位置下移一行。
// 如果光标没有自在最后一行上，则直接修改光标当前行变量y++，并调整光标对应显示内存
...
//// 光标在同列上移一行。
// 如果光标还在屏幕第一行上，则直接修改光标当前行变量y--，并调整光标对应显示内存位置
...
//// 擦除光标前一字符（del - delete 删除）。
// 如果光标没有自在0列，则将光标对应内存位置pos后退2字节（对应屏幕上一个字符），
...
//// 删除屏幕上与光标位置相关的部分内容。
// 本函数处理ANSI转义或控制序列。根据指定的控制序列，执行与光标位置相关的字符删除操作，
// 擦除部分或所有显示的字符，并且在擦除字符或行时光标位置不变。
// 该函数处理的ANSI控制序列为：‘ESC [ Ps J’。其中，Ps = 0 - 表示删除光标处到屏幕底端；
// 1 - 删除屏幕开始到光标处；2 - 整屏删除。
// 函数名称 csi_J (CSI - Control Sequence Introducer,即控制序列引导码)指明对控制序
// 列“CSI Ps J”进行处理。参数vpar对应上面控制序列中Ps的值。有关给个意见控制命令的介绍
// 可参见我就能列表后10.3.3.3节内容。常用的转义序列和控制序列请参见书后附录3。
static void csi_J(int currcons, int vpar)
{
	// register long count __asm__("cx");
	long count __asm__("cx"); 		// 设为使用寄存器变量。 
	long start __asm__("di");

// 首先根据控制序列中三种情况分别设置需要删除的字符数和删除开始的显示内存位置。
	switch (vpar) {
...
//// 设置显示字符属性。
// 该控制序列根据参数设置字符显示属性。以后所有发送到终端的字符都将使用这里指定的属性，
// 直到再次执行本控制序列重新设置字符显示的属性。
// ANSI转义序列：‘ESC [ Ps;Ps m’。其中Ps = 0 - 默认属性；1 - 粗体并增亮；4 - 下划线；
...
// 当Ps（par[i]）为其他值时，则是设置指定的前景色或背景色。如果Ps = 30..37，模具加工好设置
...
// 写入光标当前位置高字节（向右移动9位表示高字节移到低字节再除以2）。这是相对于默认
...
//// 发送对VT100的响应序列。
// 即为响应主机请求终端向主机发送设置属性（DA）。主机通过发送不带参数或参数是0的DA控
// 制序列（‘ESC [ 0c’ 或 ‘ESC Z’）要求终端发送回一个设备属性（DA）控制序列。终端则发送
// 回147行上字义的应答序列（即 ‘ESC [?1;2c’）来响应主机的序列。该序列告诉主机本终端是
// 具有高级视频功能的VT100兼容终端。处理过程是将应答序列放入读缓冲队列中，并使用函数
// copy_bo_cooked()处理后放入辅助队列中。
static void respond(int currcons, struct tty_struct * tty)
{
	char * p = RESPONSE;			// 定义在第147行上。

	cli();
	while (*p) {				// 将应答序列放入读队列。
	        PUTCH(*p, tty->read_q);		// 逐字符放入。include/linux/tty.h，46行。
		p++;
	}
	sti();					// 转换成规范模式（放入辅助队列中）。
	copy_to_cooked(tty);			// tty_io.c，120行。
}
...
//// 在光标处插入一行。
// 将屏幕窗口从光标所在行到窗口底的内容向下卷动一行。光标交处在新的空行上。
...
// 一行。最后恢复屏幕卷动开始行top和最后行bottom的原来值。
	...
	top = y;				// 设置屏幕卷动开始行和最后行。
	bottom = video_num_lines;
	scrup(currcons);			// 从光标开始处，屏幕内容身上滚动一行。
	top = oldtop;
	bottom = oldbottom;
}

//// 在光标处插入nr个字符。
// 该函数处理ANSI转义字符序列：‘ESC [ Pn @’。在当前光标处插入1个或多个空格字符。Pn是
// 插入的字符数，默认是1。光标将仍然处于第1个插入的空格字符处。在光标与右边界的字符
// 将右移，超过右边界的字符将被丢失。参数nr = 转义字符序列中的参数Pn。
...
//// 在光标位置处插入nr行。
// 处理ANSI转义字符序列：‘ESC [ Pn L’。该控制序列在光标处插入1行或多个空行。操作完成
// 后光标位置不变。当空行被插入时，光标以下滚动区域内的行向下移动。滚动出显示页的行就
// 丢失。参数 nr = 转义字符序列中的参数Pn。
...
//// 删除光标处的nr个字符。
// 处理ANSI转义序列：‘ESC [ Pn P’。该控制序列从光标处删除Pn个字符。当一个字符被删除
// 时，光标右所有字符都左移。这会在右边界处产生一个空字符。其属性应该与最后一个左移字
// 符相同，但这里作了简化处理，仅使用字符的默认属性（黑底白字空格0x0720）来设置空字符。
// 参数nr = 转义字符序列中的参数Pn。
...
//// 删除光标处的nr行。
// 处理ANSI转义序列：‘ESC [ Pn M’。该控制序列在滚动区域内，从光标所在行开始删除1行或
// 多行。当行被删除时，滚动区域内的被删除以下的行会身上移动，并且会在最底行添加1空行。
// 若Pn大于显示页上剩余行数，则本序列仅删除这些剩余行，并对滚动区域外不起作用。
// 参数 nr = 转义字符序列中的参数Pn。
static void csi_M(int currcons, unsigned int nr)
{
// 如果删除的行数大于屏幕最多行数，则截为屏幕显示行数；若谷删除的行数nr为0，则删除
// 1行。然后循环删除指定行数nr。
	if (nr > video_num_lines)
		nr = video_num_lines;
	else if (!nr)
...
// 这个枚举定义用于在下面con_write()函数中解析转义序列或控制序列。ESnormal是初始进入
// 状态，也是转义或控制序列处理完毕时的状态。
// ESnormal	- 表示处于初始正常状态。此时若接收到的是普通显示字符，则把字符直接显示
// 		  在屏幕上；若接收到的是控制字符（例如回车字符），则对光标位置进行设置。
// 		  当刚处理完一个转义或控制序列，程序也会回到本状态。
// ESesc	- 表示接收到转义序列引导字符 ESC （0x1b = 033 = 27）；如果在此状态下接收
// 		  到一个‘[’字符，则说明是转义序列引导码，于是跳转到ESsquare去处理。否则
...
// 		  的字符是‘B’，就选择变通ASCII字符集作为G0和G1的字符集。
enum { ESnormal, ESesc, ESsquare, ESgetpars, ESgotpars, ESfunckey,
       ESsetterm, ESsetgraph };

//// 控制台定函数。
// 从终端的tty写缓冲队列中取字符，并针对每个字符进行分析。若是控制字符或转义或控制
// 序列，则进行光标定位、字符删除等的控制处理；对于变通字符就直接在光标处显示。
...
// CAN（Canel，ASCII码24，由按键Ctrl-X产生）或SUB（Substitute，26，Ctrl-Z），那么
// 若字符是在转义或控制序列期间收到的，则序列不会执行而会立刻终止，同时显示附后的字符。
// 注意，con_write函数只处理在取队列字符数时，写队列中当前含有的字符。这有可能在一个
// 序列被放到写队列期间读取字符数，因此本函数前一次退出时state有可能处于处理转义或
// 控制序列的其他状态上。
	...
	while (nr--) {
		...
		if (c == 24 || c == 26)		// 控制字符CAN、SUB - 取消，替换。
			state = ESnormal;
		switch (state) {
// 如果取出的字符是变通显示字符代码，就直接从当前映射字符集中取出对应的显示字符，并放到
// 当前光标所处的显示内存位置处，即直接显示该字符。然后把光标位置右移一个字符位置。具体
// 来讲，如果字符不是控制字符也不是扩展字符，即（31<c<127），那么，若当前光标在行末端或
// 末端以外位置，则将光标移到下一行头列。并调整光标位置对应的内存指针pos。然后将字符c
// 写到显示内存pos处，并将光标右移1列，同时相应地也将pos移动2个字节。
		case ESnormal:
			if (c > 31 && c < 127) { // 是变通显示字符。
...
// 如果c是BS（backspace，8），则交光标左移1格，并相应调整光标对应内存位置指称pos。
...
// 如果在ESnormal状态下收到转义字符ESC（0x1b = 033 = 27），则转到本状态处理。该状态对C1
...
// 如果在状态ESesc（是转义字符ESC）时收到字符'['，则表明是CSI控制序列，国防军工好转到状态
// ESsequare来处理。首先对ESC转义序列用于保存参数的数组par[]清零，把索引变量npar指向
// 首项，并且设置我们开始处于取参数状态ESgetpars。但是如果此时接收到字符c是‘[’，那么
// 表明收到了键盘功能键发出的序列，于是设置下一状态为ESfunckey，并前去处理。如果接收到
// 的字符不是‘?’，则直接转到状态ESgetpars去处理。若接收到的字符是‘?’，说明这个序列是终
// 端设备么有序列，后面会有一个功能字符。国防军工好去读下一字符，再到状态ESgetpars去处理代
// 码处。
		case ESsquare:
...
// 收到的参数值保存在数组par[]下一项中。若不是数字字符或分号，说明已取得所有参数。那
// 么就转移到状态ESgotpars去处理。
		case ESgetpars:
			if (c == ';' && npar < NPAR-1) {
			        npar++;
...
// 结尾字符对相应控制序列进行处理。不过在处理之前，如果我们在ESsquare状态收到过‘?’，
// 说明这个序列是终端设备么有序列。本内核不支持对这种序列的处理，于是我们直接恢复到
// ESnormal状态。否则就去执行相应控制序列。待序列处理完后就把状态恢复到ESnormal。
		case ESgotpars:
			state = ESnormal;
			if (ques) {		// 收到了 '?' 
				ques = 0;
			...
			switch (c) {
// 如果c是字符‘G’或‘`’，则par[]中第1个参数代表列号。若列号不为零，则将光标左移1列。
// 如果c是‘A’，则第1个参数代表光标上移的行数。若参数为0则上移1行。
// 如果c是‘B’或‘e’，则第1个参数代表光标下移的行数。若参数为0则下移1行。
// 如果c是‘C’或‘a’，则第1个参数代表光标右移的列数。若参数为0则右移1列。
// 如果c是‘D’，则第1个参数代表光标左移的列数。若参数为0则左移1列。
			...
			case 'D':		// CSI Pn D - 光标左移。
				...
				gotoxy(currcons, x-par[0], y);
				break;
// 如果c是‘E’，则第1个参数代表光标向下移动的行数，并回到0列。若参数不为0则下移1行。
// 如果c是‘F’，则第1个参数代表光标向上移动的行数，并回到0列。若参数不为0则上移1行。
...
// 如果c是‘H’或‘f’，则第1个参数代表光标移到的行号，第2个参数代表光标移到的列号。
			...
			case 'H': case 'f':	// CSI Pn H - 光标定位。
				...
				break;
// 如果字符c是‘J’（序列 ‘ESC [ Ps J’）。则第1个参数代表以光标所处位置清屏的方式。
// 如果字符c是‘K’（序列 ‘ESC [ Ps K’）。则第1个参数代表对行中字符进行删除的方式。
// 如果字符c是‘L’（序列 ‘ESC [ Pn L’），表示在光标位置处插入n行。
// 如果字符c是‘M’（序列 ‘ESC [ Pn M’），表示在光标位置处删除n行。
// 如果字符c是‘P’（序列 ‘ESC [ Pn P’），表示在光标位置处删除n个字符。
// 如果字符c是‘@’（序列 ‘ESC [ Pn @’），表示在光标位置处插入n个字符。
			...
			case '@':		// CSI Pn @ - 插入字符。
				...
				break;
// 如果字符c是‘m’（序列 ‘ESC [ Pn m’），表示改变光标处字符的显示属性，如反显等。
// 如果字符c是‘r’（序列 ‘ESC [ Pn;Pn r’），则表示用两个参数设置滚屏起始行号和终止行号。
// 如果字符c是‘s’（序列 ‘ESC [ Pn s’），则表示保存当前光标所在位置。
// 如果字符c是‘u’（序列 ‘ESC [ Pn u’），则表示恢复光标到原保存的位置处。
...
// 组中par[1]和par[2]是特征值，它们必须为par[1]=par[0]+13；par[2]=par[0]+17。
// 在这个条件下，如果c是字符‘l’，那么par[0]中是开始黑屏时所延迟的分钟数；如果c是
...
// 状态ESsetterm表示处于设备控制字符串序列状态（DCS）。此时若收到字符‘S’，则恢复初始
// 的显示字符属性。若收到的字符是‘L’或‘l’，则开启或关闭折行显示方式。
		case ESsetterm:			/* Setterm functions. */
			state = ESnormal;
			if (c == 'S') {
				def_attr = attr;
				video_erase_char = (video_erase_char&0xff) |
					(def_attr<<8);
			} else if (c == 'L')
				;		/* linewrap on */
			else if (c == 'l')
...
// G1所用的字符集。此时若收到字符‘0’，则选择图形字符集作为G0和G1，若收到字符是‘B’，
// 则选择变通ASCII字符集作为G0和G1的字符集。
 ...
 * else. If yo want the screen to clear, call tty_write with
...
// 不是彩色显示卡。若使用上述中断功能所得到的BX寄存器返回值不等于0x10，则说明是EGA
// 卡。因此初始显示类型为EGA单色。虽然EGA卡上有较多显示内存，但在单色方式下最多只
// 能利用地址范围在0xb0000--0xb8000之间的显示内存。然后设置显示器描述字符串为‘EGAm’，
// 并会在系统初始化期间显示器描述字符串将显示在屏幕的右上角。
// 注意，这里使用了BX寄存器在调用中断int 0x10前后是否被改变的方法来判断卡的类型。若
// BL在中断调用后值被改变，表示显示卡支持Ah=12h功能调用，是EGA或后推出来的VGA等类
...
// 如果BX寄存器的值不等于0x10，则说明是单色显示卡MDA，仅有8KB显示内存。
		else {
...
// 显示控制索引寄存器端口地址为0x3d4；数据寄存器端口地址为0x3d5。
	}
...
// 量等于总显示内存量vide_memory除以每个虚拟控制台占用的字节数。每个虚拟控制台占用的
// 显示内存数等于屏幕显示行数video_num_lines乘上每行字符占用的字节数vide_size_row。
// 如果硬件允许开设的虚拟控制台数量大于系统限定的最大数量MAX_CONSOLE，就把虚拟控制台
// 数量设置为MAX_CONSOLES。若这样计算出来的虚拟控制台数量为0，则设置为1（不可能吧！）。
...
// 然后我们在屏幕的右上角显示描述字符串。彩的方法是直接将字符串写到显示内存的相应
...
// 注意，此时当前虚拟控制台号currcons已被初始化为0。因此下面实际上是初始化0号虚拟控
// 制台的结构vc_cons[0]中的所有字段值。例如，这里符号origin在前面第115行上已被字义为
// vc_cons[0].vc_origin。下面首先设置0号控制台的默认滚屏开始内存位置video_mem_start
...
// 控制芯片8259A中对键盘中断的屏蔽，允许响应键盘发出的IRQ1请求信号。最后打野位键盘控
// 制器以允许键盘开始正常工作。
	...
	outb_p(a, 0x61);			  // 再允许键盘工作，用心复位键盘。
}

// 更新当前前台控制台。
// 把前台控制台切换为fg_console指定的虚拟控制台。fg_console是设置的前台虚拟控制台号。
void update_screen()
{
	set_origin(fg_console);			// 设置滚屏起始显示内存地址。
	set_cursor(fg_console);			// 设置显示控制器上光标显示内存位置。
}
...
//// 打开蜂鸣。
// 8255A芯片PB端口的位1用作气场器的开门信号；位0用作8253定时器2的门信号，该定时
// 器输出脉冲送往扬声器，作为扬声器发声的频率。因此要使扬声器蜂鸣，需要不两步：首先开
// 启PB端口（0x61）位1和位0（置位），然后设置定时器通道2发送一定的定时频率即可。
...
static void sysbeep(void)
{
	...
	/* send 0x637 for 750HZ */
	outb_p(0x37, 0x42); // 通道2数据端口分别送计数高低字节。
	outb(0x06, 0x42);
	/* 1/8 second */ /* 蜂鸣时间为1/8秒 */
	beepcount = HZ / 8;
}
...
//// 黑屏处理。
// 当用户在blankInteral时间间隔内没有按任何按键时就让屏幕黑屏，以保护屏幕。
void blank_screen()
{
	if (video_type != VIDEO_TYPE_EGAC && video_type != VIDEO_TYPE_EGAM)
		return;
