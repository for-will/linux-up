...
// 参数：gate_addr - 描述符地址；type - 打磨符类型域值；dpl - 描述符特权级；addr - 偏移地址。
...
// &idt[n]是中断描述符表中中断n对应项的偏移值；中断描述我们的的类型是14，特权级是0。
...
// 中断处理过程能够被所有程序执行。例如音频调试、溢出出错和边界超出出错处理。
// 参数：n - 中断号；addr - 中断程序偏移地址。
...
#define _set_seg_desc(gate_addr,type,dpl,base,limit) {\
	*(gate_addr) = ((base) & 0xff000000) |	/* 描述符低4字节 */ 	\
		(((base) & 0x00ff0000)>>16) | 				\
		((limit) & 0xf0000) | 					\
		((dpl)<<13) | 						\
		(0x00408000) | 						\
		((type)<<8); 						\
		*((gate_addr)+1) = (((base) & 0x0000ffff)<<16) | /* 描述符高4字节 */ \
			   ((limit) & 0x0ffff); }
...
//  |006	| \
//  |007________| \
//  |010 byte-1	| / 过程入口点偏移值 bit-0 ~ bit-15 
//  |011	| /
//  |012	| /
//  |013	| /
//  |014	| /
...
//  |050 byte-5	| \
//  |051	| \
//  |052	| / 描述符类型（0x0E - 中断门描述符；0x0F - 陷阱门描述符）。
//  |053	| /
//  |054	|_/
//  |055	| \ DPL bit-0
//  |056	|_/ DPL bit-1
//  |057________|_/ P(存在标志)
//  |060 byte-6	| \
//  |061	| \
//  |062	| \
//  |063	| \
...
//  |066	| \
//  |067________| \
//  |070 byte-7	| / 过程入口占偏移值 bit-16 ~ bit-31 
//  |071	| /
//  |072	| /
//  |073	| /
//  |074	| /
