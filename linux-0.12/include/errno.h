 ...
 * possible error number, I was forced to use the same numbers
 * as minix.
 * Hopefully these are posix or something. I wouldn't know (and posix
 * isn't telling me - they want $$$ for their f***ing standard).
 *
 * We don't use the _SIG cludge of minix, so kernel returns must
 ...
 * 相同的出错号了。
 * 希望这些是POSIX兼容的或者在一定程度上是这样的，物不知道（而且POSIX
...
// 系统调用以及很多库函数返回一个特殊的值心表示操作失败或出错。这个值通常选择-1或者
// 其他一些特定的值来表示。但是这个返回值仅说明错误发生了。如果需要知道出错的类型，
// 就需要查看表示系统出错号的变量errno。该变量即在errno.h文件中声明。在程序开始执行
// 时该该变量值被初始化为0。
extern int errno;
