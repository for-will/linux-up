...
// 第6--108行量该文件第1部分。其中主要定义了目标文件执行结构以及相关操作的宏定义。
// 目标文件结构。参见程序后的详细说明。
//============================== 
...
struct exec {
	...
	unsigned a_text;	/* lenght of text, in bytes */
	unsigned a_data;	/* length of data, in bytes */
	unsigned a_bss;		/* length of uninitialized data area for file, in bytes */
	unsigned a_syms;	/* length of symbol table data in file, in bytes */
	unsigned a_entry;	/* start address */
	unsigned a_trsize;	/* length of relocation info for text, in bytes */
	unsigned a_drsize;	/* lenght of relocation info for data, in bytes */
};
...
// 这里，Linux0.12内核把内存页定义为4KB，段大小定义为1KB。因此没有使用上面的字义。
#define PAGE_SIZE 4096
...
// 第110--185行是第2部分。对目标文件中的符号表项和相关操作宏进行字义和说明。
...
// 下面3个常量定义是nlist结构中n_type字段的屏蔽码（八进制表示）。
#ifndef N_EXT
...
#define N_STAB 0340				/* 0xe0(0b1110,0000)这几个比特用于符号调度器 */
#endif
...
/* 下面的类型指明一个符号的字义是作为另一个符号的间接引用。紧接该
   符号后面的其他的符号呈现为未定义的引用。

   这种间接引用是不对称的。另琴符号的值将被用于满足间接符号的要求，
   但反之则不然。如果另琴符号没有定义，则将搜索库来寻找一个定义 */
#define N_INDR 0xa
...
/* 下面的符号与集合元素有关。所有具有相同名称N_SET[ATDB]的符号
   ...
   的值存放在空间的一个字（word）中。空间的第一个字是集合的长度（元素的数目）。

   集合的地址被放入一个N_SETV符号中，它的名称与集合同名。
   该符号的行为像一个N_DATA全局符号，因为它可以满足未定义的外部引用。 */
...
/* 下面的结构描述了要执行的单个重宝子们操作。
   文件代码重定位们部分是这些结构的一个数组，所有这些结构都适用于代码部分。
   同样，数据重定位部分用于数据部分。 */
...
struct relocation_info
{
	...
	/* 非零表示值是一个pc相对偏移量，因而在其自己地址空间
	   以及符号或指定的节部分改变时，它需要被重定位 */
	unsigned int r_pcrel:1;
	/* Lenght (as exponent of 2) of the field to be relocated.
