 ...
 * 命令实际上还没有测试过。
 */
// 文件句柄（描述符）操作函数fcntl()使用的命令（cmd）。
#define F_DUPFD		0	/* dup ; 复制文件句柄为最小数值的句柄。 */
#define F_GETFD		1	/* get f_flags ; 取句柄标志。仅1个标志FD_CLOEXEC。 */
#define F_SETFD		2	/* set f_flags ; 设置文件句柄标志。 */
#define F_GETFL		3	/* more flags (cloexec) ; 取文件状态标志和访问械。 */
