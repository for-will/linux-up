...
// 等待输出队列处理完毕（空），若参数值是0，则发送一个break（参见tcsendbrek()，tcdrain()）。
...
// 设置终端串行线路专用模式。名称TIOC[*]的含义是tty输入输出控制命令。
...
// 则是关闭的。为了能使用这两个ioctl调用，tty线路应该是以O_NDELY方式打开的，这样
// open()就不会等待载波。
...
struct termios {
	tcflag_t c_iflag;	/* inputmode flags */		// 输入模式标志。
...
// 以下是控制字符数组c_cc[]中项的索引值。该数组初始值字义在include/linux/tty.h中。
// 程序可以更改这个数组中的值。如果定义了_POSIX_VDISABLE（\0），那么当数组某一项值
// 等于_POSIX_VDISABLE的值时，表示禁止使用数组中相应的特殊字符。
/* c_cc characters */
...
#define VDISCARD 13  /* c_cc[VDISCARD]	= DISCARD  (^0), \017, 丢弃字符。 		*/
...
// termios结构中输出模式字段c_oflsg各种标志的符号常数。
/* c_oflag bits */
...
#define ONLRET	0000040		/* 换行符NL执行回车符CR */
...
#define NOFLSH	0000200		/* 当生成SIGINT和SIGQUIT信号时不刷新输入输出队列，当
				   生成SIGSUSP信号时，刷新输入队列 */
#define TOSTOP	0000400		/* 发送SIGTTOU信号到后台进程的进程组，该后台进程试图写
				   自己的控制终端 */
#define ECHOCTL	0001000 	/* 若设置了ECHO，则除TAB、NL、START和STOP以外的ASCII
				   控制字符将被回显成象^X式样，X值是控制符+0x40 */
...
#define TIOCM_CAR	0x040	/* 载波监测（Carier Detect） */
