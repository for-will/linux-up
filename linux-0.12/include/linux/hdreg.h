 ...
 */
/*
 * 本文件含有一些AT硬盘控制器的字义。来自各种资料。主查证某些
 * 定义（带有问号的注释）。
 */
#ifndef _HDREG_H
#define _HDREG_H
...
#define HD_PRECOMP HD_ERROR	/* some io address, read=error, write=precomp */
#define HD_COMMAND HD_STATUS	/* same io adress, read=status, write=cmd */

#define HD_CMD          0x3f6	/* 控制寄存器端口 */

/* Bits of HD_STATUS */
/* 硬盘状态寄存器各位的字义（HD_STATUS） */
...
#define READY_STAT      0x40	/* 驱动器准备好了（就绪） */
...
#define TRKO_ERR	0x02	/* couldn't find trak 0 */
