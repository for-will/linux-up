 ...
 * offset into 'tty_queue')
 */
/*
 * ‘tty’中字义了tty_io.c程序使用的某些结构和其他一些定义。
 *
 * 注意！在修改这里的字义时，一定要检查rs_io.s或con_io.s程序中不会出现问题。
...
// tty字符缓冲随便一个数据结构。用于tty_struct结构中的读、写和辅助（规范）缓冲队列。
struct tty_queue {
...
// 以下字义了tty等待队列中缓冲区操作宏。（tail在前，head在后，参见tty_io.c的图）。
...
// 从queue队列项缓冲区中tail处取一字符，并且tail+=1。
#define GETCH(queue, c) \
	(void)({c=(queue)->buf[(queue)->tail];INC((queue)->tail);})
// 往queue队列项缓冲区中head处放置一字符，并且让head+=1。
...
// 根据终端商类型在tty_table[]中取对应终端号nr的tty结构指针。第73行后半部分用于根据
// 子设备号dev在tty_table[]表中选择对应的tty结构。如果dev = 0，表示正在使用前台终端。
// 因此可以直接使用终端号fg_console作为tty_table[]索引取tty结构。如果dev大于0，那么
// 就要分两种情况考虑。①dev是虚拟终端号；②dev是串行终端号或者伪终端号。对于虚拟终端，
// 其tty结构在tty_table[]中索引项是dev-1（0-63）。对于其它类型终端，则它们的tty结构
// 索引项就是dev。例如，如果dev=64，表示是一串行终端1，则其tty结构就是tty_table[dev]。
// 如果dev = 1，则对应终端的tty结构是tty_table[0].参见tty_io.c程序第70 -- 73行。
#define TTY_TABLE(nr) \
(tty_table + ((nr) ? (((nr) < 64) ? (nr)-1 : (nr)) : fg_console))

// 这里给出了终端termios结构中可更改的特殊字符数组c_cc[]的初始值。POSIX.1定义了11个
// 特殊字符，但是Linux还另外定义了SVR4使用的6个特殊字符。若字义了_POSIX_VDISABLE(\0)，
// 那么当某一项值等于_POSIX_VDISABLE的值时，表示禁止使用相应的特殊字符。在下面81行上这
// 些初始值使用8进制表示。
 ...
 * 文件结束eof=^D	vtime=\0	vim=\1		sxtc=\0
 * 开始start=^Q		标识卡stop=^S	挂起susp=^Z	行结束eol=\0
