...
#ifndef _TIME_T
#define _TIME_T
typedef long time_t;				// 从GMT 1970年1月1日午夜0时开始计的时间（秒）。
#endif
...
typedef long clock_t;				// 从进程开始执行计起的系统经过的时钟测滴答数。
...
// 以下是有关时间操作的函数原型。
// 确定字处理器使用时间。返回程序所用处理器时间（滴答数）的近似值。
clock_t clock(void);
...
// 将日历时间转换成一个字符串形式，如“Wed Jun 30 30 21:49:08:1993\n”
struct tm * ctime(const time_t * tp);
// 将日历时间转换成tm结构表示的UTC时间（UTC - 世界时间代码Universal Time Code）。
struct tm * gmtime(const time_t * tp);
// 将日历时间转换成tm结构表示的指定时区（Time Zone）的时间。
struct tm * localtime(const time_t * tp);
// 利用格式字符串fmt将tm结构表示的时间转换成最大长度为smax的字符串并将结果存储在s中。
